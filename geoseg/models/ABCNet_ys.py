...
from geoseg.Net.ABCNet.resnet import Resnet18
...
class FeatureAggregationModule(nn.Module):
    ...
    def forward(self, fsp, fcp):
        ...
        # print("fcp",fcp.shape)
        # print('----------------------------------')
        # batch_size, chnnels, width, height = fsp.shape
        # fsp = fsp.view(batch_size, chnnels,height,width)
        fcat = torch.cat([fsp, fcp], dim=1)
        # batch_size, chnnels, width, height = fcat.shape
        # fcat = fcat.view(batch_size, chnnels, height, width)
...
class ABCNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # 2023.02.09新增模块
        feat_out = F.interpolate(feat_out, size=(H, W), mode='bilinear', align_corners=False)
